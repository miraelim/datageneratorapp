...
int main(int argc, char **argv)
{
    ...
    FILE *fp1, *humiditydatapoint;
    char weatherdata[1024];
    struct tm *t;
    time_t timer;

    fp1 =fopen("weatherdata.txt","r");
    humiditydatapoint =fopen("humiditydatapoint.txt","w+");
	...
	json_object_object_add(schemaobj, "name", json_object_new_string("weather_humidity"));
	...
	fprintf(humiditydatapoint, "%s\n", json_object_to_json_string(myobj));
    }
    fclose(fp1);
    fclose(humiditydatapoint);
    return 0;
}
