...
int main(int argc, char **argv)
{
    ...
    json_object *humidity, *humiditybody, *humiditydata;
    ...
    char temperaturedata[1024], humidata[1024];
    tempdatapoint =fopen("tempdatapoint.txt","r");
    humiditydatapoint =fopen("humiditydatapoint.txt","r");

    struct tm *t;
    time_t timer;

    if(tempdatapoint==NULL){
	printf("temperaturedatapoint 파일 읽기  실패\n");
	return 0;
    }
    if(humiditydatapoint==NULL){
	printf("humiditydatapoint 파일 읽기  실패\n");
	return 0;
    }
	...
	char *ft, *fh;
	ft = fgets(temperaturedata, 1024, tempdatapoint);
	fh = fgets(humidata, 1024,humiditydatapoint);
	if( ft == NULL || fh == NULL)
	    break;
	...
	humidity = json_tokener_parse(humidata);
	tempheader = json_object_object_get(temperature, "header");
	tempbody = json_object_object_get(temperature, "body");

	//	humidityheader = json_object_object_get(humiditydata, "header");
	humiditybody = json_object_object_get(humidity, "body");
	...
	humiditydata = json_object_object_get(humiditybody, "humidity");
	humidityunit = json_object_object_get(humiditydata, "unit");

	dval = json_object_object_get(humiditydata, "value");	
	...
	json_object_object_add(schemaobj, "name", json_object_new_string("weather_merge(T/H)"));
